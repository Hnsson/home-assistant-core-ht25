...
def validate_entity_config(values: dict) -> dict[str, dict]:
    ...
    for entity_id, config in values.items():
        ...
        entities[entity] = _validate_domain_config(domain, entity, config)

    return entities


def _validate_domain_config(domain: str, entity: str, config: dict) -> dict:
    """Validate config based on the entity domain."""
    ...
    if domain == "fan":
        return FAN_SCHEMA(config)
    if domain == "sensor":
        return SENSOR_SCHEMA(config)
    if domain == "valve":
        return VALVE_SCHEMA(config)
